...
(tauy, ady) = AllanDeviation(thetay, fs)
...
slopen = -0.5
logtau = np.log10(tauy)
logadev = np.log10(ady)
...
# Plot data on log-scale
tauB = tauy[i]
lineB = B * scfb * np.ones(np.size(tauy))

# plt.figure()
# plt.title('GyroY Allan Deviations - Location D')
# plt.plot(tauy, ady, label=r'$\sigma$ (rad/sec)')
# plt.plot(tauy, lineN, label = r'$\sigma_{N}$ (rad/sec)/$\sqrt{Hz}$', linestyle='dashed')
...
# plt.plot(tauy, lineK, label = r'$\sigma_{K}$ (rad/sec)/$\sqrt{Hz}$', linestyle='dashed')
...
# plt.plot(tauy, lineB, label = r'$\sigma_{B}$ (rad/sec)', linestyle='dashed')
...
# df2.plot(x="time(s)", y=["GyroY", "gyroY", "gyroZ"],
        # kind="line", title="Gyro x,y,z vs Time Location A", ylabel='gyro (rad/s)').set_xlim(xmin=0, xmax=(time[len(time)-1]+1))

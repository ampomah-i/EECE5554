...
vectornav_string_list = LocationA_df['data'].values.tolist()
time = LocationA_df['Time'].values.tolist()
print(len(time))
...
(tauz, adz) = AllanDeviation(thetaz, fs)
...
slopen = -0.5
logtau = np.log10(tauz)
logadev = np.log10(adz)
...
# Plot data on log-scale
tauB = tauz[i]
lineB = B * scfb * np.ones(np.size(tauz))

plt.figure()
plt.title('GyroZ Allan Deviations - Location D')
plt.plot(tauz, adz, label=r'$\sigma$ (rad/sec)')
plt.plot(tauz, lineN, label = r'$\sigma_{N}$ (rad/sec)/$\sqrt{Hz}$', linestyle='dashed')
...
plt.plot(tauz, lineK, label = r'$\sigma_{K}$ (rad/sec)/$\sqrt{Hz}$', linestyle='dashed')
...
plt.plot(tauz, lineB, label = r'$\sigma_{B}$ (rad/sec)', linestyle='dashed')
plt.plot(tauB, scfb*B, 'o', ms=15, mec='red', mfc='none', mew=2)
plt.annotate('0.664B',xy=(tauB, scfb*B), xytext=(0.1, scfb*B+50),
   textcoords='offset points',
   color='black', size='large', arrowprops=dict(
      arrowstyle='simple,tail_width=0.3,head_width=0.8,head_length=0.8',
      facecolor='b', shrinkB=10))
...
# df2.plot(x="time(s)", y=["GyroY", "gyroY", "gyroZ"],
        # kind="line", title="Gyro x,y,z vs Time Location B", ylabel='gyro (rad/s)').set_xlim(xmin=0, xmax=(time[len(time)-1]+1))
